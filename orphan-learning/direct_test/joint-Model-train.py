...
train_protein = prepare_data(data_dir,"./data/train_whole_sps",vocab_size_protein,vocab_protein,protein_MAX_size,1)
train_compound = prepare_data(data_dir,"./data/train_whole_smile",vocab_size_compound,vocab_compound,comp_MAX_size,0)
train_IC50 = read_labels("./data/train_whole_ic50")
'''
dev0_protein = prepare_data(data_dir,"./data/dev0_sps",vocab_size_protein,vocab_protein,protein_MAX_size,1)
dev0_compound = prepare_data(data_dir,"./data/dev0_smile",vocab_size_compound,vocab_compound,comp_MAX_size,0)
dev0_IC50 = read_labels("./data/dev0_ic50")

dev1_protein = prepare_data(data_dir,"./data/dev1_sps",vocab_size_protein,vocab_protein,protein_MAX_size,1)
dev1_compound = prepare_data(data_dir,"./data/dev1_smile",vocab_size_compound,vocab_compound,comp_MAX_size,0)
dev1_IC50 = read_labels("./data/dev1_ic50")

dev2_protein = prepare_data(data_dir,"./data/dev2_sps",vocab_size_protein,vocab_protein,protein_MAX_size,1)
dev2_compound = prepare_data(data_dir,"./data/dev2_smile",vocab_size_compound,vocab_compound,comp_MAX_size,0)
dev2_IC50 = read_labels("./data/dev2_ic50")

dev3_protein = prepare_data(data_dir,"./data/dev3_sps",vocab_size_protein,vocab_protein,protein_MAX_size,1)
dev3_compound = prepare_data(data_dir,"./data/dev3_smile",vocab_size_compound,vocab_compound,comp_MAX_size,0)
dev3_IC50 = read_labels("./data/dev3_ic50")
'''
test0_protein = prepare_data(data_dir,"./data/PXR_sps",vocab_size_protein,vocab_protein,protein_MAX_size,1)
test0_compound = prepare_data(data_dir,"./data/PXR_smile",vocab_size_compound,vocab_compound,comp_MAX_size,0)
test0_IC50 = read_labels("./data/PXR_ic50")

test1_protein = prepare_data(data_dir,"./data/REV_sps",vocab_size_protein,vocab_protein,protein_MAX_size,1)
test1_compound = prepare_data(data_dir,"./data/REV_smile",vocab_size_compound,vocab_compound,comp_MAX_size,0)
test1_IC50 = read_labels("./data/REV_ic50")
'''
...
fc_3 = fully_connected(drop_3, num3_neurons, activation='leakyrelu',weights_init="xavier",name='fully3')
...
print("error on dev0")
size = 64
length_dev0 = len(dev0_protein)
print(length_dev0)
num_bins = math.ceil(length_dev0/size)
for i in range(num_bins):
        if i==0:
          y_pred = model.predict([dev0_protein[0:size],dev0_compound[0:size]])
        elif i < num_bins-1:
          temp = model.predict([dev0_protein[(i*size):((i+1)*size)],dev0_compound[(i*size):((i+1)*size)]])
          y_pred = np.concatenate((y_pred,temp), axis=0)
        else:
          temp = model.predict([dev0_protein[(i*size):length_dev0],dev0_compound[(i*size):length_dev0]])
          y_pred = np.concatenate((y_pred,temp), axis=0)
row = np.arange(length_dev0)
col = np.argmax(y_pred, axis=1)
data = np.ones(length_dev0)
y_pred_label = coo_matrix((data, (row, col)), shape=y_pred.shape).toarray()
dev0_01 = dev0_IC50[:, 0:2]
dev0_3 = dev0_IC50[:, -1]
dev0_24 = np.zeros((length_dev0, 1))
dev0_IC50 = np.column_stack((np.column_stack((np.column_stack((dev0_01, dev0_24)), dev0_3)), dev0_24))
print(diagnosis(dev0_IC50, y_pred_label))


print("error on dev1")
size = 64
length_dev1 = len(dev1_protein)
print(length_dev1)
num_bins = math.ceil(length_dev1/size)
for i in range(num_bins):
        if i==0:
          y_pred = model.predict([dev1_protein[0:size],dev1_compound[0:size]])
        elif i < num_bins-1:
          temp = model.predict([dev1_protein[(i*size):((i+1)*size)],dev1_compound[(i*size):((i+1)*size)]])
          y_pred = np.concatenate((y_pred,temp), axis=0)
        else:
          temp = model.predict([dev1_protein[(i*size):length_dev1],dev1_compound[(i*size):length_dev1]])
          y_pred = np.concatenate((y_pred,temp), axis=0)
row = np.arange(length_dev1)
col = np.argmax(y_pred, axis=1)
data = np.ones(length_dev1)
y_pred_label = coo_matrix((data, (row, col)), shape=y_pred.shape).toarray()
dev1_01 = dev1_IC50[:, 0:2]
dev1_34 = dev1_IC50[:, 2:]
dev1_2 = np.zeros((length_dev1, 1))
dev1_IC50 = np.column_stack((np.column_stack((dev1_01, dev1_2)), dev1_34))
print(diagnosis(dev1_IC50, y_pred_label))


print("error on dev2")
size = 64
length_dev2 = len(dev2_protein)
print(length_dev2)
num_bins = math.ceil(length_dev2/size)
for i in range(num_bins):
        if i==0:
          y_pred = model.predict([dev2_protein[0:size],dev2_compound[0:size]])
        elif i < num_bins-1:
          temp = model.predict([dev2_protein[(i*size):((i+1)*size)],dev2_compound[(i*size):((i+1)*size)]])
          y_pred = np.concatenate((y_pred,temp), axis=0)
        else:
          temp = model.predict([dev2_protein[(i*size):length_dev2],dev2_compound[(i*size):length_dev2]])
          y_pred = np.concatenate((y_pred,temp), axis=0)
row = np.arange(length_dev2)
col = np.argmax(y_pred, axis=1)
data = np.ones(length_dev2)
y_pred_label = coo_matrix((data, (row, col)), shape=y_pred.shape).toarray()
dev2_01 = dev2_IC50[:, 0:2]
dev2_3 = dev2_IC50[:, -1]
dev2_24 = np.zeros((length_dev2, 1))
dev2_IC50 = np.column_stack((np.column_stack((np.column_stack((dev2_01, dev2_24)), dev2_3)), dev2_24))
print(diagnosis(dev2_IC50, y_pred_label))


print("error on dev3")
size = 64
length_dev3 = len(dev3_protein)
print(length_dev3)
num_bins = math.ceil(length_dev3/size)
for i in range(num_bins):
        if i==0:
          y_pred = model.predict([dev3_protein[0:size],dev3_compound[0:size]])
        elif i < num_bins-1:
          temp = model.predict([dev3_protein[(i*size):((i+1)*size)],dev3_compound[(i*size):((i+1)*size)]])
          y_pred = np.concatenate((y_pred,temp), axis=0)
        else:
          temp = model.predict([dev3_protein[(i*size):length_dev3],dev3_compound[(i*size):length_dev3]])
          y_pred = np.concatenate((y_pred,temp), axis=0)
row = np.arange(length_dev3)
col = np.argmax(y_pred, axis=1)
data = np.ones(length_dev3)
y_pred_label = coo_matrix((data, (row, col)), shape=y_pred.shape).toarray()
dev3_01 = []
for i in range(length_dev3):
    if dev3_IC50[i] == 0:
        dev3_01.append(1)
        dev3_01.append(0)
    else:
        dev3_01.append(0)
        dev3_01.append(1)
dev3_01 = np.array(dev3_01).reshape((length_dev3, 2))
dev3_234 = np.zeros((length_dev3, 3))
dev3_IC50 = np.column_stack((dev3_01, dev3_234))
print(diagnosis(dev3_IC50, y_pred_label))
...
y_pred_label = coo_matrix((data, (row, col)), shape=y_pred.shape).toarray()
test0_0 = test0_IC50[:, 0]
test0_3 = test0_IC50[:, 1]
test0_12 = np.zeros((length_test0, 2))
test0_4 = np.zeros((length_test0, 1))
test0_IC50 = np.column_stack((np.column_stack((np.column_stack((test0_0, test0_12)), test0_3)), test0_4))
...
y_pred_label = coo_matrix((data, (row, col)), shape=y_pred.shape).toarray()
test1_0 = test1_IC50[:, 0]
test1_3 = test1_IC50[:, 1]
test1_12 = np.zeros((length_test1, 2))
test1_4 = np.zeros((length_test1, 1))
test1_IC50 = np.column_stack((np.column_stack((np.column_stack((test1_0, test1_12)), test1_3)), test1_4))
