...
train_protein = prepare_data(data_dir,"./data/train_final_sequence",vocab_size_protein,vocab_protein,protein_MAX_size,0)
...
test0_protein = prepare_data(data_dir,"./data/m_sequence",vocab_size_protein,vocab_protein,protein_MAX_size,0)
test0_compound = prepare_data(data_dir,"./data/m_compound",vocab_size_compound,vocab_compound,comp_MAX_size,0)
test0_IC50 = read_labels("./data/m_ic50")
...
prot_gru_1 = tflearn.gru(prot_embd, GRU_size_prot,trainable=True,return_seq=True,restore=True)
prot_gru_1 = tf.stack(prot_gru_1,axis=1)
prot_gru_2 = tflearn.gru(prot_gru_1, GRU_size_prot,trainable=True,return_seq=True,restore=True)
...
drug_gru_1 = tflearn.gru(drug_embd,GRU_size_drug,trainable=True,return_seq=True,restore=True)
drug_gru_1 = tf.stack(drug_gru_1,1)
drug_gru_2 = tflearn.gru(drug_gru_1, GRU_size_drug,trainable=True,return_seq=True,restore=True)
...
######## training
model.fit([train_protein,train_compound], {'target': train_IC50}, n_epoch=500,batch_size=64,
