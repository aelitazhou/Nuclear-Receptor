...
def read_initial_state_weigths(path,size1,size2):
    ...
    for line in f:
       ...
       if len(y) == size2 and count < size1:
          x.append(y)
          count = count+1
       #else:
          #print("not exactly equal to size2!!!!!!", len(y), count)
...
'''
train_protein = prepare_data(data_dir,"./data/train_10_sequence",vocab_size_protein,vocab_protein,protein_MAX_size,0)
train_compound = prepare_data(data_dir,"./data/train_10_smile",vocab_size_compound,vocab_compound,comp_MAX_size,0)
train_IC50 = read_labels("./data/train_10_ic50")

test0_protein = prepare_data(data_dir,"./data/test_sequence",vocab_size_protein,vocab_protein,protein_MAX_size,0)
test0_compound = prepare_data(data_dir,"./data/test_smile",vocab_size_compound,vocab_compound,comp_MAX_size,0)
...
prot_gru_1 = tflearn.gru(prot_embd, GRU_size_prot,trainable=False,return_seq=True,restore=True)
prot_gru_1 = tf.stack(prot_gru_1,axis=1)
prot_gru_2 = tflearn.gru(prot_gru_1, GRU_size_prot,trainable=False,return_seq=True,restore=True)
...
drug_gru_1 = tflearn.gru(drug_embd,GRU_size_drug,trainable=False,return_seq=True,restore=True)
drug_gru_1 = tf.stack(drug_gru_1,1)
drug_gru_2 = tflearn.gru(drug_gru_1, GRU_size_drug,trainable=False,return_seq=True,restore=True)
drug_gru_2 = tf.stack(drug_gru_2,axis=1)


W = tflearn.variables.variable(name="Attn_W_prot",shape=[GRU_size_prot,GRU_size_drug],initializer=tf.random_normal([GRU_size_prot,GRU_size_drug],stddev=0.1),trainable=False, restore=True)
b = tflearn.variables.variable(name="Attn_b_prot",shape=[protein_MAX_size,comp_MAX_size],initializer=tf.random_normal([protein_MAX_size,comp_MAX_size],stddev=0.1),trainable=False, restore=True)
...
U_size = 256
U_prot = tflearn.variables.variable(name="Attn_U_prot",shape=[U_size,GRU_size_prot],initializer=tf.random_normal([U_size,GRU_size_prot],stddev=0.1),trainable=False, restore=True)
U_drug = tflearn.variables.variable(name="Attn_U_drug",shape=[U_size,GRU_size_drug],initializer=tf.random_normal([U_size,GRU_size_drug],stddev=0.1),trainable=False, restore=True)
B = tflearn.variables.variable(name="Attn_B",shape=[U_size],initializer=tf.random_normal([U_size],stddev=0.1),trainable=False, restore=True)
...
conv_1 = conv_1d(Attn_reshape, 64, 4,2, activation='leakyrelu', weights_init="xavier",regularizer="L2",name='conv1', trainable=False, restore=True)
...
for v in tf.global_variables():
   ...
   elif "GRU_1/GRU_1/GRUCell/Gates/Lineainable=False, restore=Truer/Bias" in v.name :
...
fc_1 = fully_connected(pool_2, num1_neurons, activation='leakyrelu',weights_init="xavier",name='fully1', trainable=False, restore=True)
drop_2 = dropout(fc_1, drop_out)
fc_2 = fully_connected(drop_2, num2_neurons, activation='leakyrelu',weights_init="xavier",name='fully2', trainable=False, restore=True)
...
linear = fully_connected(drop_4, 5, activation='softmax',name='fully5')
classification = regression(linear, optimizer='adam', learning_rate=learning_rate,
                     loss='categorical_crossentropy', name='target')

# Training
model = tflearn.DNN(classification, tensorboard_verbose=0,tensorboard_dir='./mytensor/',checkpoint_path="./newcheckpoints/")

model.load('checkpoints-27500')

#model2 = tflearn.DNN(linear, session = model.session)


######## training
model.fit([train_protein,train_compound], {'target': train_IC50}, n_epoch=500,batch_size=8,
