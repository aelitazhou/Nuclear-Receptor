...
######## training
model.fit([protein_train,compound_train], {'target': IC50_train}, n_epoch=300,batch_size=64,
...
for i in range(num_bins):
        if i==0:
          y_pred = model.predict([protein_train[0:size],compound_train[0:size]])
        elif i < num_bins-1:
          temp = model.predict([protein_train[(i*size):((i+1)*size)],compound_train[(i*size):((i+1)*size)]])
          y_pred = np.concatenate((y_pred,temp), axis=0)
        else:
          temp = model.predict([protein_train[length_train-size:length_train],compound_train[length_train-size:length_train]])
...
y_pred_label = coo_matrix((data, (row, col)), shape=y_pred.shape).toarray()
print(diagnosis(IC50_train, y_pred_label))
